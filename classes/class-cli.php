...
class Mai_AskNews_CLI {
	 ...
	 *
	 * Usage: wp maiasknews update_matchups --posts_per_page=10 --offset=0
	 *
	 * @since 0.1.0
	 *
	 ...
	 * @return void
	 */
	function update_matchups( $args, $assoc_args ) {
		// Parse args.
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) : $query->the_post();
				// Skip if top level.
				if ( 0 === wp_get_post_parent_id( get_the_ID() ) ) {
					WP_CLI::log( 'Skipping top level post ID: ' . get_the_ID() . ' ' . get_permalink() );
					continue;
				}
